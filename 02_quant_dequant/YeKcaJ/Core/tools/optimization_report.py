...
def table(rounds, fmt, prefix=""):
    lines = ["| 轮次 | 元素数 | median ms | P95 ms | 相对上一轮 | 相对第1轮 |",
             "|---|---:|---:|---:|---:|---:|"]
    first = previous = None
    for number, (label, path, stage) in enumerate(rounds, 1):
        ...
        for n in SIZES:
            ...
            lines.append(f"| 第{number}轮：{label} | {n//1048576}M | {value:.6f} | "
                         f"{current[n]['p95_ms']:.6f} | {ratio} | {first[n]['median_ms']/value:.2f}x |")
        previous = current
    lines += ["", "数据来源（按轮次）：", ""]
