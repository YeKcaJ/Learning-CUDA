...
def comparison(fmt, previous_path, current_path, first_path, direct=False):
    previous, current, first = (rows(p, fmt) for p in (previous_path, current_path, first_path))
    middle = rows(DIRECT, fmt) if direct else None
    lines = ["| 元素数 | 上一轮 median | " + ("仅直接编码 | " if direct else "")
             + "本轮 median | 本轮 P95 | 相对上一轮 | 相对首次优化后 |",
             "|---:|---:|" + ("---:|" if direct else "") + "---:|---:|---:|---:|"]
    for n in SIZES:
        p, c, f = previous[n]["median_ms"], current[n]["median_ms"], first[n]["median_ms"]
        lines.append(f"| {n//1048576}M | {p:.6f} | "
                     + (f"{middle[n]['median_ms']:.6f} | " if middle else "")
                     + f"{c:.6f} | {current[n]['p95_ms']:.6f} | {p/c:.2f}x | {f/c:.2f}x |")
    return "\n".join(lines)
...
def render():
    ...
    lines += ["", "来源：" + source("第0次完整数据", BASE) + "。", "",
              ...
              comparison("mxfp8", BASE, MX_FIRST, MX_FIRST, direct=True), "",
              ...
              comparison("nvfp4", BASE, NV_FIRST, NV_FIRST), "",
              ...
              comparison("nvfp4", NV_FIRST, REDUCE, NV_FIRST), "",
              "来源：" + source("上一轮及首次优化后", NV_FIRST) + "、" + source("本轮", REDUCE) + "。当轮另测的 before 不替换上一轮保存值。", "",
              ...
              comparison("mxfp8", MX_FIRST, VECTOR, MX_FIRST), "",
              "来源：" + source("上一轮及首次优化后", MX_FIRST) + "、" + source("本轮", VECTOR) + "。", "",
    ...
    lines += ["", "来源：" + source("同批次新建与复用数据", WORKSPACE) + "。", "",
              ...
              comparison("nvfp4", REDUCE, PARTIAL, NV_FIRST), "",
              "来源：" + source("上一轮", REDUCE) + "、" + source("首次优化后", NV_FIRST) + "、" + source("本轮", PARTIAL) + "。原日志用当轮 before 算的1.18x/1.02x/1.01x，不是这里“相对上一轮保存值”的分母。", "",
