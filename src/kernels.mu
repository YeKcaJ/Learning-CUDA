...
namespace
{
constexpr size_t kAlign = 256;  //musaMalloc 基址对齐粒度，兼容任意向量化访存
...
class DeviceArena
{
    ...
    bool reserve(size_t bytes)
    {
        ...
        release();
        RUNTIME_CHECK(musaMalloc(reinterpret_cast<void**>(&base_), want));
        capacity_ = want;
        return true;
    }
    ...
    char* base_ = nullptr;
    size_t capacity_ = 0;
    //析构时刻意不释放：静态对象销毁可能晚于 MUSA runtime 卸载，
    //那时调用 musaFree 会报错。进程退出由驱动统一回收。
};
...
//4.拷贝数据到GPU
    RUNTIME_CHECK(musaMemcpyAsync(d_input, h_input.data(), bytes_total,
                                  musaMemcpyHostToDevice, ctx.stream));
    RUNTIME_CHECK(musaMemcpyAsync(d_weight, h_weight.data(), bytes_weight,
                                  musaMemcpyHostToDevice, ctx.stream));
...
//6.拷贝结果回CPU
    RUNTIME_CHECK(musaMemcpyAsync(h_output.data(), d_output, bytes_total,musaMemcpyDeviceToHost, ctx.stream));
    //非默认流上的异步拷贝不阻塞 host，须显式同步后 h_output 才可读
    RUNTIME_CHECK(musaStreamSynchronize(ctx.stream));
}
...
//4.拷贝数据到GPU
    RUNTIME_CHECK(musaMemcpyAsync(d_q, h_q.data(), q_bytes,
                                  musaMemcpyHostToDevice, ctx.stream));
    RUNTIME_CHECK(musaMemcpyAsync(d_k, h_k.data(), k_bytes,
                                  musaMemcpyHostToDevice, ctx.stream));
    RUNTIME_CHECK(musaMemcpyAsync(d_v, h_v.data(), v_bytes,
                                  musaMemcpyHostToDevice, ctx.stream));
    //arena 会被复用，上一次调用的残留必须清掉
    RUNTIME_CHECK(musaMemsetAsync(d_o, 0, o_bytes, ctx.stream));
...
//7.拷回结果并同步。显存由 arena 持有、跨调用复用，此处不释放
    RUNTIME_CHECK(musaMemcpyAsync(h_o.data(), d_o, o_bytes,
                                  musaMemcpyDeviceToHost, ctx.stream));
    RUNTIME_CHECK(musaStreamSynchronize(ctx.stream));
}
